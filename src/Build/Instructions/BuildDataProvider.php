...
namespace Centeron\Grids\Build\Instructions;
 ...
 *
 * This class is a build instruction for Centeron/build package
 * that defines how to setup grids data provider
 *
 * @internal
 * @package Centeron\Grids\Build\Instructions
 */
    ...
    public function apply(Scaffold $scaffold)
    {
        ...
        if (is_object($src)) {
            if (is_a($src, '\Illuminate\Database\Eloquent\Builder')) {
                $class = '\Centeron\Grids\EloquentDataProvider';
                $arg = $src;
            } elseif (is_a($src, '\Doctrine\DBAL\Query\QueryBuilder')) {
                $class = '\Centeron\Grids\DbalDataProvider';
                $arg = $src;
            }
